                            ...
                            <li className='mb-2'>
                                {JSON.parse(`[${shoe.color}]`).map((item, index) => (
                                    <Button onClick={() => setColor(item.id)}
                                        type='text' className={`border border-warning me-2 ${color === item.id && 'bg-blue'}`}>
                                        {item.name}
                                    </Button>
                                ))}
                                {color !== null &&
                            ...
                            <li className='mb-2'>
                                {JSON.parse(`[${shoe.size}]`).map((item, index) => (
                                    <Button onClick={() => setSize(item.id)}
                                        type='text' className={`border border-warning me-2 ${size === item.id && 'bg-blue'}`}>
                                        {item.name}
                                    </Button>
                                ))}
                                {size !== null &&
                            ...
                            <li className='mb-2'>
                                ...
                                <Form onFinish={handleChoose}>
                                    <Row gutter={10}>
                                        ...
                                        <Col xl={12}>
                                            <Button htmlType='submit' type='primary' className='bg-blue w-100' icon={<i className='fas fa-cart-plus'></i>}>Thêm vào giỏ hàng</Button>
                                        </Col>
                                    </Row>
                                </Form>
                            </li>
