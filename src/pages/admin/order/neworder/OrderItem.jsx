...
function OrderItem({ index, props, onSuccess }) {
  ...
  const handleCreate = async () => {
    ...
    if (paymentMethod === 2) {
      ...
      // Tiền mặt OK, tiến hành thanh toán
      ...
      // Kiểm tra tiền khách đưa cho trường hợp không thanh toán qua tiền mặt hoặc chuyển khoản
      if (typeOrder === 0 && !waitPay) {
        if (tienKhachDua <= 0 || tienKhachDua === null) {
          toast.error('Vui lòng nhập đủ tiền khách đưa!');
          return;
        }
        if (tienKhachDua < totalMoney) {
          toast.error('Tiền khách đưa phải >= tổng tiền cần thanh toán!');
          return;
        }
      }
      // Thực hiện thao tác tạo hóa đơn
      Modal.confirm({
        title: "Xác nhận",
        maskClosable: true,
        ...
        onOk: async () => {
          try {
            const response = await request.put(`/bill/${props.id}`, data);
  ...
  return (
    <>
      ...
      <div style={{ boxShadow: "2px 2px 4px 4px rgba(0, 0, 0, 0.03)" }} className="my-3 p-2 mt-4">
        ...
        <Row gutter={10}>
          <Col xl={14}>
            {typeOrder === 0 ? <img src="/giay1.png" width={"100%"} alt="" /> : (
              <>
          ...
          <Col xl={10} md={24}>
            <ul className="list-unstyled">
              ...
              <li>
                <Button type="primary" className="bg-blue text-dark w-100" onClick={() => handleCreate()}>Tạo hóa đơn</Button>
              </li>
            </ul>
          </Col>
        </Row>
