...
function ShowProductModal({ idBill, onClose }) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (id, record) => (
                <>
                    <Button type='primary' className='bg-blue text-dark' onClick={() => handleChoose(record)}>Chọn</Button>
                </>
            )
        },
    ]
    return (
        <>
            <Button type="primary" onClick={() => setIsModalOpen(true)} className="bg-blue text-dark">Thêm sản phẩm</Button>
