      ...
      render: (x, record) => (
        <>
          ...
          <Tooltip placement="bottom" title="Xóa">
            <Button type="text" onClick={() => handleDelete(record.id)}><i className="fas fa-trash text-danger"></i></Button>
          </Tooltip>
        </>
      )
    },
      ...
      <BaseUI>
        ...
        <div className="d-flex">
          ...
          {selectedRowKeys.length > 0 && (
            <>
              ...
              <div className="">
                <Button type="primary" onClick={() => handleUpdateFast()} className="bg-blue">Cập nhật {selectedRowKeys.length} sản phẩm</Button>
              </div>
            </>
          )}
        </div>
        <Form layout='vertical' onFinish={(data) => setDataFilter(data)} form={formFilter}>
          ...
          <div className="text-center">
            ...
            <Button htmlType='submit' className='bg-blue text-dark' type='primary' icon={<i className='fas fa-search'></i>}>Tìm kiếm</Button>
          </div>
        </Form>
