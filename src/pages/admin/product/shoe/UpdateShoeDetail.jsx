...
function UpdateShoeDetail({ props, onSuccess }) {
    ...
    const handleOk = (data) => {
        console.log("Data to update:", data); // Log dữ liệu cần cập nhật
        data.shoe = props.id;
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const response = await request.put(`/shoe-detail/${props.id}`, data);
                    console.log("Update response:", response); // Log phản hồi từ API
                    toast.success('Cập nhật thành công!');
                    setIsModalOpen(false);
                    onSuccess(); // Gọi hàm callback khi thành công
                } catch (e) {
                    console.error("Update error:", e); // Log lỗi nếu có
    ...
    return (
        <>
            ...
            <Modal title={props.name} open={isModalOpen} onCancel={handleCancel} footer={
                <>
                    <Button type='primary' htmlType='submit' form='formUpdate' className='bg-blue'>Cập nhật</Button>
                </>
            } width={800}>
